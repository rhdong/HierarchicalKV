...
void test_multi_tables_on_multi_threads(size_t max_hbm_for_vectors,
  ...
  auto worker_function = [&max_hbm_for_vectors, &use_constant_memory](
    ...
    while (table->load_factor() < 1.0) {
      ...
      CUDA_CHECK(cudaStreamSynchronize(stream));
      table->insert_or_assign(first ? 0 : KEY_NUM*2, d_keys, d_vectors, nullptr, stream);
      first = false;
      CUDA_CHECK(cudaStreamSynchronize(stream));
...
TEST(MerlinHashTableTest, test_multi_tables_on_multi_threads) {
  ...
  test_multi_tables_on_multi_threads(2, false);
}
...
//}
//TEST(MerlinHashTableTest, test_multi_tables_on_multi_threads2) {
//  test_multi_tables_on_multi_threads(16, true);
//  test_multi_tables_on_multi_threads(2, false);
//}
...
//}
//TEST(MerlinHashTableTest, test_multi_tables_on_multi_threads3) {
//  test_multi_tables_on_multi_threads(16, true);
//  test_multi_tables_on_multi_threads(2, false);
//}
