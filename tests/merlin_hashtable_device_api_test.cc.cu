...
#include <array>
#include "merlin/hashtable_device_api.cuh"
...
__global__ void ReadonlyLookupKernel(
    ...
    int key_count, bool* founds, int* positions) {
  cg::thread_block_tile<TILE_SIZE> tile =
      cg::tiled_partition<TILE_SIZE>(cg::this_thread_block());
  const int tile_global = blockIdx.x * (blockDim.x / TILE_SIZE) +
  ...
  const int pos =
      nv::merlin::device::readonly_lookup_no_sync<K, V, S, TILE_SIZE>(
...
TEST(HashTableDeviceApiTest, ReadonlyLookup) {
  ...
  CUDA_CHECK(cudaMemcpy(d_values, h_values.data(),
                        ...
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_scores, h_scores.data(),
                        sizeof(S) * insert_count, cudaMemcpyHostToDevice));
  ...
  ReadonlyLookupKernel<kTileSize>
      <<<blocks, threads>>>(view, d_query, query_count, d_founds, d_positions);
  CUDA_CHECK(cudaDeviceSynchronize());

  std::array<bool, query_count> h_founds{};
  std::array<int, query_count> h_positions{};
  CUDA_CHECK(cudaMemcpy(h_founds.data(), d_founds,
