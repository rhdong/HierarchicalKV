...
//    ASSERT_EQ(total_size, 0);
//
//    table->accum_or_assign(KEY_NUM, d_keys, d_vectors, d_scores, stream);
//    CUDA_CHECK(cudaStreamSynchronize(stream));
//
//    total_size = table->size(stream);
//    CUDA_CHECK(cudaStreamSynchronize(stream));
...
//  CUDA_CHECK(cudaStreamDestroy(stream));
//
//  CUDA_CHECK(cudaMemcpy(h_vectors, d_vectors, KEY_NUM * sizeof(V) *
//  options.dim,
//                        cudaMemcpyDeviceToHost));
//
//  CUDA_CHECK(cudaFreeHost(h_keys));
      ...
      for (int i = 0; i < dump_counter; i++) {
        ...
        for (int j = 0; j < TEST_KEY_NUM; j++) {
          if (h_keys_test[j] == h_keys_temp[i]) {
            is_found_on_base = h_found_temp[j];
        ...
        if (is_found_on_base && is_accum_test)
          expected_score = base_score + test_score;
...
void test_evict_strategy_customized_advanced(size_t max_hbm_for_vectors) {
  ...
  h_accum_or_assigns_test[7] = true;
...
void test_accum_or_assign_values_check(size_t max_hbm_for_vectors) {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    CheckAccumOrAssignValues<K, V, S, dim>(table.get(), data_buffer, B, stream);
