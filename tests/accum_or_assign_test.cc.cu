...
void test_basic_when_full(size_t max_hbm_for_vectors) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    table->accum_or_assign(KEY_NUM, d_keys, d_vectors, d_metas, stream);
    CUDA_CHECK(cudaStreamSynchronize(stream));
...
void test_rehash_on_big_batch(size_t max_hbm_for_vectors) {
  ...
  table->accum_or_assign(INIT_KEY_NUM, d_keys, d_vectors, d_metas, stream);
  CUDA_CHECK(cudaStreamSynchronize(stream));
...
void CheckAccumOrAssignValues(Table* table, K* keys, V* values, M* metas,
  ...
  table->accum_or_assign(len, keys, values, nullptr, stream);
  CUDA_CHECK(cudaStreamSynchronize(stream));
...
TEST(AccumOrAssignTest, test_accum_or_assign_multi_threads) {
  test_accum_or_assign_multi_threads(16, 0.25f, 0.125f);
  test_accum_or_assign_multi_threads(16, 0.375f, 0.125f);
  test_accum_or_assign_multi_threads(0, 0.25f, 0.125f);
  test_accum_or_assign_multi_threads(0, 0.375f, 0.125f);
}
...
TEST(AccumOrAssignTest, test_basic_for_cpu_io) { test_basic_for_cpu_io(); }
