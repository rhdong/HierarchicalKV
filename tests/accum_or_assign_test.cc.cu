 ...
 * the accum_or_assign API is safe to use:
 *
 ...
 */
void test_accum_or_assign_basic() {
  TableOptions opt;
  ...
  table->find(
      M, buffer.keys_ptr(), nullptr, buffer.status_ptr(), nullptr, stream);
  table->accum_or_assign(
      M, buffer.keys_ptr(), buffer.values_ptr(), buffer.status_ptr(), nullptr, stream);

  size_t table_size_m = table->size(stream);
  buffer.SyncData(/*h2d=*/false, stream);
  evict_buffer.SyncData(/*h2d=*/false, stream);
  ASSERT_EQ(table_size_m, M);
  ...
  table->find(
      M, buffer.keys_ptr(), nullptr, buffer.status_ptr(), nullptr, stream);
  table->accum_or_assign(
      N, buffer.keys_ptr(), buffer.values_ptr(), buffer.status_ptr(), buffer.metas_ptr(), stream);
  size_t table_size_n = table->size(stream);
  buffer.SyncData(/*h2d=*/false, stream);
  evict_buffer.SyncData(/*h2d=*/false, stream);
  for (size_t i = 0; i < table_size_n; i++) {
    test_util::ValueArray<f32, dim>* vec =
        reinterpret_cast<test_util::ValueArray<f32, dim>*>(
            evict_buffer.values_ptr(false) + i * dim);
    summarized_kvs.emplace(evict_buffer.keys_ptr(false)[i], *vec);
...
void CheckAccumOrAssign(Table* table, K* keys, V* values, M* metas,
  ...
  size_t filtered_len =
      table->accum_or_assign(len, keys, values, nullptr, evicted_keys,
...
void test_accum_or_assign_advanced() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    CheckAccumOrAssign<i64, f32, u64>(
...
void test_accum_or_assign_with_export_batch() {
  ...
  size_t n_evicted = table->accum_or_assign(
...
template <typename K, typename V, typename M>
void BatchCheckAccumOrAssign(Table* table, K* keys, V* values, M* metas,
                              K* evicted_keys, V* evicted_values,
                              M* evicted_metas, size_t len,
  ...
  while (step->load() < total_step) {
    ...
    CUDA_CHECK(cudaStreamSynchronize(stream));
    auto start = std::chrono::steady_clock::now();
    size_t filtered_len = table->accum_or_assign(
...
void test_accum_or_assign_run_with_batch_find() {
  ...
  std::thread accum_or_assign_thread;
  ...
  auto accum_or_assign_func = [&table, &global_buffer, &evict_buffer, &B,
                                &step, &batch_num, &insert_stream]() {
    BatchCheckAccumOrAssign<i64, f32, u64>(
  ...
  auto find_func = [&table, &global_buffer, &B, &step, &batch_num,
    ...
    BatchCheckFind<i64, f32, u64>(
        table.get(), global_buffer.keys_ptr(), global_buffer.values_ptr(),
        global_buffer.metas_ptr(), B, &step, batch_num, find_interval,
        find_stream, if_check);
  };

  find_thread = std::thread(find_func);
  accum_or_assign_thread = std::thread(accum_or_assign_func);
  find_thread.join();
  accum_or_assign_thread.join();
  CUDA_CHECK(cudaStreamDestroy(insert_stream));
  CUDA_CHECK(cudaStreamDestroy(find_stream));
}

TEST(AccumOrAssignTest, test_accum_or_assign_basic) {
  test_accum_or_assign_basic();
}

TEST(AccumOrAssignTest, test_accum_or_assign_advanced) {
  test_accum_or_assign_advanced();
}

TEST(AccumOrAssignTest, test_accum_or_assign_with_export_batch) {
  test_accum_or_assign_with_export_batch();
}

TEST(AccumOrAssignTest, test_accum_or_assign_run_with_batch_find) {
  test_accum_or_assign_run_with_batch_find();
}
