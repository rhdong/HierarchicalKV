 ...
 public:
  ...
  void find_or_insert(const size_type n, const key_type* keys,  // (n)
    ...
    if (is_fast_mode()) {
      ...
      const size_type dev_ws_size{n * (sizeof(value_type*) + sizeof(int) +sizeof(bool))};
      auto dev_ws{dev_mem_pool_->get_workspace<1>(dev_ws_size, stream)};
...

      {
        ...
        find_or_insert_kernel<key_type, value_type, meta_type, TILE_SIZE>
            <<<grid_size, block_size, 0, stream>>>(d_table_, keys, d_table_value_addrs, metas,
...

      {
        ...
        thrust::sort_by_key(thrust_par.on(stream), table_value_ptr, table_value_ptr + n,
                            param_key_index_ptr, thrust::less<uintptr_t>());
      }

      if (options_.io_by_cpu) {
        const size_type host_ws_size{dev_ws_size +
                                     n * (sizeof(bool) + sizeof(value_type) * dim())};
        auto host_ws{host_mem_pool_->get_workspace<1>(host_ws_size, stream)};
        ...
        CUDA_CHECK(cudaMemcpyAsync(h_table_value_addrs, d_table_value_addrs, dev_ws_size,
                                   cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaMemcpyAsync(h_founds, founds, n * sizeof(bool),
                                   cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaMemcpyAsync(h_param_values, values, n * sizeof(value_type) * dim(),
                                   cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaStreamSynchronize(stream));

        read_or_write_by_cpu<value_type>(h_table_value_addrs, h_param_values, h_param_key_index, 
                                         h_founds, dim(), n);
        CUDA_CHECK(cudaMemcpyAsync(values, h_param_values, n * sizeof(value_type) * dim(),
        ...
        read_or_write_kernel<key_type, value_type, meta_type>
            <<<grid_size, block_size, 0, stream>>>(d_table_value_addrs, values, founds,
                                                   param_key_index, dim(), N);
      }
    }
    ...
    if (is_fast_mode()) {
...

      {
        ...
        update_kernel<key_type, value_type, meta_type, TILE_SIZE>
            <<<grid_size, block_size, 0, stream>>>(d_table_, keys, d_dst, metas, d_src_offset, N);
      }
