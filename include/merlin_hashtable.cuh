...
class HashTable {
 ...
 public:
  ...
  void insert_or_assign(const size_type n,
    ...
    if (is_fast_mode()) {
      ...
      Selector::execute_kernel(load_factor, options_.block_size,
                               ...
                               dim(), stream, n, d_table_, keys, values,
                               scores);
    } else {
...

      {
        ...
        upsert_kernel<key_type, value_type, score_type, TILE_SIZE>
            <<<grid_size, block_size, 0, stream>>>(
                ...
                keys, d_dst, scores, d_src_offset, N);
      }
  ...
  void insert_and_evict(const size_type n,
    ...
    Selector::execute_kernel(load_factor, options_.block_size,
                             options_.max_bucket_size, table_->buckets_num,
                             dim(), stream, n, d_table_, keys, values, scores,
  ...
  void accum_or_assign(const size_type n,
    ...
    if (is_fast_mode()) {
...

      {
        ...
        accum_or_assign_kernel<key_type, value_type, score_type>
            <<<grid_size, block_size, 0, stream>>>(
                d_table_, options_.max_bucket_size, table_->buckets_num, dim(),
                keys, dst, scores, accum_or_assigns, src_offset, founds, N);
      }
    ...
    if (is_fast_mode()) {
...

      {
        ...
        find_or_insert_kernel<key_type, value_type, score_type, TILE_SIZE>
            <<<grid_size, block_size, 0, stream>>>(
                d_table_, options_.max_bucket_size, table_->buckets_num, dim(),
                keys, d_table_value_addrs, scores, founds, param_key_index, N);
      }
    ...
    if (is_fast_mode()) {
...

      {
        ...
        lookup_kernel<key_type, value_type, score_type, TILE_SIZE>
            <<<grid_size, block_size, 0, stream>>>(
                d_table_, options_.max_bucket_size, table_->buckets_num, dim(),
                keys, src, scores, founds, dst_offset, N);
      }
