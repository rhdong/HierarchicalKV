...
/* If founds[i] = true, read data from corresponding address of table_value_addrs and write to param_values;
 * if founds[i] = false, write data from param_values to corresponding address of table_value_addrs.
 * usually called by find_or_insert kernel.
 */
template <class V>
void read_or_write_by_cpu(V** __restrict table_value_addrs, V* __restrict param_values,
                  const int* __restrict offset, const bool* founds, size_t dim, int N,
                  int n_worker = 16) {
  std::vector<std::thread> thds;
  if (n_worker < 1) n_worker = 1;

  auto functor = [=](V** __restrict table_value_addrs, V* __restrict param_values,
                    const int* __restrict offset, int handled_size,
                    int trunk_size) -> void {
    for (int i = handled_size; i < handled_size + trunk_size; i++) {
      if (founds[offset[i]] && table_value_addrs[i] != nullptr) {
        memcpy(param_values + offset[i] * dim, table_value_addrs[i], sizeof(V) * dim);
      }
      else {
        memcpy(table_value_addrs[i], param_values + offset[i] * dim, sizeof(V) * dim);
      }
    }
  };
  ...
  for (int i = 0; i < n_worker - 1; i++) {
    thds.push_back(
        std::thread(functor, table_value_addrs, param_values, offset, handled_size, trunk_size));
    handled_size += trunk_size;
  }

  size_t remaining = N - handled_size;
  thds.push_back(
      std::thread(functor, table_value_addrs, param_values, offset, handled_size, remaining));
 ...
 */
template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__global__ void find_or_insert_kernel(const Table<K, V, M>* __restrict table,
                              const K* __restrict keys, V** __restrict vectors,
                              M* __restrict metas, bool* __restrict found,
  ...
  for (size_t t = tid; t < N; t += blockDim.x * gridDim.x) {
    ...
    const K param_key = keys[key_idx];
    ...
    bucket = get_key_position<K>(buckets, param_key, bkt_idx, start_idx,
                                 buckets_num, bucket_max_size);

    if (rank == 0 && keys_index != nullptr) {
      *(keys_index + key_idx) = key_idx;
    }

    found_vote = find_in_bucket<K, V, M, TILE_SIZE>(
        g, bucket->keys, param_key, tile_offset, start_idx, bucket_max_size);
    ...
    while (tile_offset < bucket_max_size && local_size < bucket_max_size) {
      ...
      found_vote = g.ballot(param_key == current_key);
      ...
      if (empty_vote) {
        ...
        if (rank == src_lane) {
          occupy_result = try_occupy<K, V, M, TILE_SIZE>(g, bucket, param_key,
                                                         current_atomic_key);
        }
    ...
    while (occupy_result == OccupyResult::CONTINUE ||
      ...
      if (rank == src_lane) {
        bucket->keys[key_pos].store(param_key,
                                    cuda::std::memory_order_relaxed);
        *(vectors + key_idx) = (bucket->vectors + key_pos * dim);
      }
...
*/
template <class K, class V, class M>
__global__ void read_or_write_kernel(V** __restrict table_value_addrs, V* __restrict param_values,
  ...
  for (size_t t = tid; t < N; t += blockDim.x * gridDim.x) {
    ...
    if (table_value_addrs[vec_index] != nullptr) {
      ...
      if (mask[real_key_index]) {
        param_values[real_key_index * dim + dim_index] = table_value_addrs[vec_index][dim_index];
      }
      /// insert
      else {
        table_value_addrs[vec_index][dim_index] = param_values[real_key_index * dim + dim_index];
      }
    }
  }
}
 ...
 */
template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__forceinline__ __device__ void update_kernel_with_io_core(
    const Table<K, V, M>* __restrict table, const K* __restrict keys,
  ...
  for (size_t t = (blockIdx.x * blockDim.x) + threadIdx.x; t < N;
    ...
    auto const found_vote = find_in_bucket<K, V, M, TILE_SIZE>(
        g, bucket->keys, update_key, tile_offset, start_idx, bucket_max_size);

    if (found_vote) {
      const int dst_lane = __ffs(found_vote) - 1;
      key_pos = (start_idx + tile_offset + dst_lane) & (bucket_max_size - 1);
      V* dst = bucket->vectors + key_pos * dim;

      if (rank == dst_lane) {
        if (metas != nullptr) {
          bucket->metas[key_pos].store(metas[key_idx], cuda::std::memory_order_relaxed);
        }
        else {
          M cur_meta = bucket->cur_meta.fetch_add(1) + 1;
          bucket->metas[key_pos].store(cur_meta, cuda::std::memory_order_relaxed);
      ...
      lock<Mutex, TILE_SIZE, true>(g, table->locks[bkt_idx]);
      copy_vector<V, TILE_SIZE>(g, update_value, dst, dim);
      unlock<Mutex, TILE_SIZE, true>(g, table->locks[bkt_idx]);
    }
    ///TODO:(jiashuy) conform: otherwise ignore it 
  }
}

template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__global__ void update_kernel_with_io(const int c_table_index_,
                                      const K* __restrict keys,
                                      const V* __restrict values, 
                                      const M* __restrict metas,
                                      size_t N) {
  const Table<K, V, M>* table = reinterpret_cast<const Table<K, V, M>*>(
      c_table_ + c_table_index_ * sizeof(Table<K, V, M>));
  update_kernel_with_io_core<K, V, M, TILE_SIZE>(table, keys, values, metas,
                                                 N);
}

template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__global__ void update_kernel_with_io(const Table<K, V, M>* table,
                                      const K* __restrict keys,
                                      const V* __restrict values, 
                                      const M* __restrict metas,
                                      size_t N) {
 ...
 */
template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__forceinline__ __device__ void find_or_insert_kernel_with_io_core(
    const Table<K, V, M>* __restrict table, const K* __restrict keys,
  ...
  for (size_t t = tid; t < N; t += blockDim.x * gridDim.x) {
    ...
    Bucket<K, V, M>* bucket = get_key_position<K>(
        buckets, find_or_insert_key, bkt_idx, start_idx, buckets_num, bucket_max_size);

    local_size = buckets_size[bkt_idx];

    auto  found_vote = find_in_bucket<K, V, M, TILE_SIZE>(
        g, bucket->keys, find_or_insert_key, tile_offset, start_idx, bucket_max_size);
    ...
    if (found_vote) {
      ...
      const V* src = bucket->vectors + key_pos * dim;
      lock<Mutex, TILE_SIZE, true>(g, table->locks[bkt_idx]);
      copy_vector<V, TILE_SIZE>(g, src, find_or_insert_value, dim);
      unlock<Mutex, TILE_SIZE, true>(g, table->locks[bkt_idx]);
    ...
    while (tile_offset < bucket_max_size && local_size < bucket_max_size) {
      ...
      found_vote = g.ballot(find_or_insert_key == current_key);
      ...
      const unsigned empty_vote =
          g.ballot(current_key == static_cast<K>(EMPTY_KEY) ||
                   current_key == static_cast<K>(RECLAIM_KEY));
      if (empty_vote) {
        const int src_lane = __ffs(empty_vote) - 1;

        key_pos = (start_idx + tile_offset + rank) & (bucket_max_size - 1);
        AtomicKey<K>* current_atomic_key = &(bucket->keys[key_pos]);

        if (rank == src_lane) {
          occupy_result = try_occupy<K, V, M, TILE_SIZE>(g, bucket, find_or_insert_key,
                                                         current_atomic_key);
        }
    ...
    while (occupy_result == OccupyResult::CONTINUE ||
           occupy_result == OccupyResult::INITIAL) {
      key_pos = bucket->min_pos.load(cuda::std::memory_order_acquire);
      const int src_lane = (key_pos % TILE_SIZE);
      OverrideResult override_result{OverrideResult::INITIAL};
      ...
      if (rank == src_lane) {
        bucket->keys[key_pos].store(find_or_insert_key,
                                    cuda::std::memory_order_relaxed);
      }
...
template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__global__ void find_or_insert_kernel_with_io(const int c_table_index_,
                                      const K* __restrict keys,
                                      V* __restrict values, 
                                      M* __restrict metas,
                                      const size_t N) {
  const Table<K, V, M>* table = reinterpret_cast<const Table<K, V, M>*>(
      c_table_ + c_table_index_ * sizeof(Table<K, V, M>));
  find_or_insert_kernel_with_io_core<K, V, M, TILE_SIZE>(table, keys, values, metas, N);
}

template <class K, class V, class M, uint32_t TILE_SIZE = 4>
__global__ void find_or_insert_kernel_with_io(const Table<K, V, M>* table,
                                      const K* __restrict keys,
                                      V* __restrict values, 
                                      M* __restrict metas,
                                      const size_t N) {
