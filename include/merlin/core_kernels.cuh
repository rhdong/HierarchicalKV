...
template <class K, class V, class M>
__global__ void set_key_meta(Bucket<K, V, M>* __restrict buckets,
...
template <class K, class V, class M>
__global__ void get_key_meta(Bucket<K, V, M>* __restrict buckets,
                             const size_t index, AtomicKey<K>** keys,
                             AtomicMeta<M>** metas) {
  Bucket<K, V, M>* __restrict bucket = buckets + index;
  *keys = bucket->keys;
...
template <class K, class V, class M>
__global__ void set_slice(V** slices, const size_t slice_index, V* slice) {
  slices[slice_index] = slice;
}

template <class K, class V, class M>
__global__ void get_slice(V** slices, const size_t slice_index, V** slice) {
  *slice = slices[slice_index];
}

template <class K, class V, class M>
__global__ void set_vectors(Bucket<K, V, M>* __restrict buckets,
                            const size_t start, const size_t end, V** slices,
...
void initialize_buckets(Table<K, V, M>** table, const size_t start,
  ...
  for (size_t i = (*table)->num_of_memory_slices;
...

    {
      ...
      set_vectors<K, V, M><<<grid_size, block_size, 0, stream>>>(
          (*table)->buckets,
...

  {
    ...
    const int grid_size = SAFE_GET_GRID_SIZE(N, block_size);
    create_locks<Mutex>
        <<<grid_size, block_size, 0, stream>>>((*table)->locks, start, end);
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }

  {
    ...
    const size_t N = end - start + 1;
    const int grid_size = SAFE_GET_GRID_SIZE(N, block_size);
    create_atomic_keys<K, V, M><<<grid_size, block_size, 0, stream>>>(
...

  {
    ...
    const size_t N = end - start + 1;
    const int grid_size = SAFE_GET_GRID_SIZE(N, block_size);
    create_atomic_metas<K, V, M><<<grid_size, block_size, 0, stream>>>(
        (*table)->buckets, start, end, (*table)->bucket_max_size);
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }
