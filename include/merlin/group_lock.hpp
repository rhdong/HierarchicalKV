...
/*
 * Considering the E2E performance, we customized a group mutex and relative lock guards:
 ...
 * - The `unique_lock` is used for special APIs (like `reserve` `erase` `clear` etc.)
 */
#include <atomic>
#include <mutex>
...
class unique_lock {
 public:
  ...
  explicit unique_lock(group_shared_mutex& mutex) : mutex_(&mutex) {
    mutex_->lock_write_read();
    owns_ = true;
  }

  explicit unique_lock(group_shared_mutex& mutex, std::defer_lock_t) noexcept
      : mutex_(&mutex), owns_(false) {}
